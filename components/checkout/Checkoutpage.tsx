...
import React, { useState, useEffect, useContext } from "react"; // Ensure React is imported
...
const Checkoutpage = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        Alert.alert("Success", responseData.message || "Order placed successfully!");
        ...
        throw new Error(responseData.message || 'Order placement resulted in an unexpected status.');
      }
  ...
  const canPlaceOrder = cartItems.length > 0 && !loading && !orderSuccess && selectedPaymentOption !== 0;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {orderSuccess && <Text style={styles.successText}>Order placed successfully!</Text>}
      ...
      <View style={styles.section}>
        ...
        {[
          ...
          >
            ...
            <Ionicons
              name={selectedPaymentOption === option.id ? "radio-button-on" : "radio-button-off"}
              size={wp("5.5%")}
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.row, styles.totalRow]}>
          ...
          <Text style={styles.totalAmountText}>{totalAmountToPay.toFixed(2)} QAR</Text>
        </View>
      </View>
