...
const Cart = () => {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      ...
      try {
        const response = await fetch(`https://alkarmah-backend.onrender.com/api/cart?userId=${user._id}`);
        if (!response.ok) {
  ...
  const handleRemoveFromCart = async () => {
    ...
    try {
      const response = await fetch('https://alkarmah-backend.onrender.com/api/cart/remove', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateQuantity = async (productId: string, amount: number) => {
    ...
    try {
      const response = await fetch('https://alkarmah-backend.onrender.com/api/cart/add', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
